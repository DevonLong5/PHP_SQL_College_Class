...
print '<h2>Login Form</h2>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if ((!empty($_POST['email'])) && (!empty($_POST['password']))){
            ...
            print '<p class="text--error"> The submitted email adress and password do not match those on file!<br>Go back and try again.</p>';
        }
    }else{
        print '<form action=:"login.php" method="post" class="form--inline">
        <p><label for "email">Email Adress:</label><input type="email" name="email" size="20"></p>
